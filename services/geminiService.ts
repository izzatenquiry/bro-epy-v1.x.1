...
const getActiveApiKey = (): string | null => {
    ...
    const appName = MASTER_CONFIG.appName || 'app';
    const sessionKey = `${appName.replace(/\s/g, '_')}_session_api_key`;
...
export const generateVideo = async (
    ...
    try {
        ...
        const videoFile = new File([blob], `${appName}-veo3-${Date.now()}.mp4`, { type: 'video/mp4' });
