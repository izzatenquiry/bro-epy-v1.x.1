...
export const downloadAsset = (item: HistoryItem, blobUrls: Map<string, string>) => {
    const appName = MASTER_CONFIG.appName?.replace(/\s/g, '-') || 'app';
    ...
    switch (item.type) {
        case 'Image':
        case 'Canvas':
            fileName = `${appName}-${item.type.toLowerCase()}-${item.id.substring(0, 8)}.png`;
            href = `data:image/png;base64,${item.result}`;
            break;
        case 'Video':
        case 'Audio':
            const extension = item.type === 'Video' ? 'mp4' : 'wav';
            fileName = `${appName}-${item.type.toLowerCase()}-${item.id.substring(0, 8)}.${extension}`;
            href = blobUrls.get(item.id) || null;
            break;
        case 'Storyboard':
        case 'Copy':
            fileName = `${appName}-${item.type.toLowerCase()}-${item.id.substring(0, 8)}.txt`;
