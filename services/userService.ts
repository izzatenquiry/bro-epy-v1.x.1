...
import { TABLE_NAMES } from './configService';
...
export const getAllUsers = async (): Promise<User[] | null> => {
    const { data, error } = await supabase.from(TABLE_NAMES.USERS).select('*');
...
export const replaceUsers = async (importedUsers: User[]): Promise<{ success: boolean; message: string }> => {
    try {
        ...
        const { error: deleteError } = await supabase.from(TABLE_NAMES.USERS).delete().not('role', 'eq', 'admin');
        if (deleteError) throw deleteError;

        const { error: insertError } = await supabase.from(TABLE_NAMES.USERS).insert(profilesToInsert);
...
export const initializeAdminAccount = async () => {
    ...
    const { error: upsertError } = await supabase.from(TABLE_NAMES.USERS).upsert(profileData, { onConflict: 'id' });
...
export const logActivity = async (
    ...
    if (!user || user.id === 'MASTER_ADMIN') {
        // Fail silently if no user or master admin. We don't want to block user actions for logging.
        console.warn('Cannot log activity: user not found or is Master Admin.');
        return;
    }
...
export const getVeoAuthTokens = async (): Promise<{ token: string; createdAt: string }[] | null> => {
    const { data, error } = await supabase
        .from(TABLE_NAMES.VEO_AUTH_TOKENS)
...
export const getSharedMasterApiKey = async (): Promise<string | null> => {
    const { data, error } = await supabase
        .from(TABLE_NAMES.MASTER_API_KEYS)
        .select('api_key')
        .order('created_at', { ascending: false })
        .limit(1)
        .single();
...
export const getAvailableApiKeys = async (): Promise<AvailableApiKey[]> => {
    const { data, error } = await supabase
        .from(TABLE_NAMES.GENERATED_API_KEYS)
...
export const incrementStoryboardUsage = async (user: User): Promise<{ success: true; user: User } | { success: false; message: string }> => {
    if (user.id === 'MASTER_ADMIN') return { success: true, user };
    try {
...
export const incrementImageUsage = async (user: User): Promise<{ success: true; user: User } | { success: false; message: string }> => {
    if (user.id.startsWith('trial-') || user.id === 'MASTER_ADMIN') {
        return { success: true, user };
    }
    try {
...
export const updateUserLastSeen = async (userId: string): Promise<void> => {
    if (userId.startsWith('trial-') || userId === 'MASTER_ADMIN') {
        return;
    }
    try {
        const { error } = await supabase
