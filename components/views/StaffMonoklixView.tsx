    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            ...
            await addHistoryItem({
                type: 'Copy',
                prompt: `Staff {appName} (${selectedAgent.name}): ${userInput.substring(0, 50)}... (Lang: ${selectedLanguage})`,
                result: result,
            });
        } catch (e) {
        ...
        <>
             {generatedCopy && !isLoading && (
                <div className="absolute top-3 right-3 flex gap-2 z-10">
                    ...
                    <button
                        onClick={() => downloadText(generatedCopy, `${MASTER_CONFIG.appName?.replace(/\s/g, '-')}-staff-${selectedAgent.id}.txt`)}
