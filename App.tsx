...
const AssigningTokenModal: React.FC<AssigningTokenModalProps> = ({ status, error, scanProgress, onRetry }) => {
    ...
    const statusInfo = {
        ...
        success: {
            ...
            message: `Your secure connection is ready. Welcome to ${appName}!`,
        },
        error: {
...
const ServerSelectionModal: React.FC<{ onSelect: (serverUrl: string) => void, currentUser: User }> = ({ onSelect, currentUser }) => {
    ...
    const servers = MASTER_CONFIG.proxyServerList || [];
...
const App: React.FC = () => {
  ...
  const handleLogout = useCallback(async () => {
    ...
    sessionStorage.removeItem(`${MASTER_CONFIG.appName}_session_api_key`); // Clean up session key
  ...
  useEffect(() => {
    const initializeSessionData = async () => {
      if (!currentUser?.id) {
        // Clear all session-specific data on logout or for master admin
        setActiveApiKey(null);
        sessionStorage.removeItem(`${MASTER_CONFIG.appName}_session_api_key`);
      ...
      try {
        ...
        const sessionApiKeyName = `${MASTER_CONFIG.appName}_session_api_key`;

        // Handle Master API Key
        if (masterKey) {
          sessionStorage.setItem(sessionApiKeyName, masterKey);
  ...
  const handleLoginSuccess = async (user: User) => {
    ...
    const hasProxyList = MASTER_CONFIG.proxyServerList && MASTER_CONFIG.proxyServerList.length > 0;

    if (hasProxyList) {
        setShowServerSelectionModal(true);
    } else {
        console.warn("[Config] No proxyServerList found in config. Proceeding without server selection.");
  ...
  const regUrl = MASTER_CONFIG.registrationUrl || '#';
  const appName = MASTER_CONFIG.appName || 'AI Platform';
  ...
  if (adminOnlyViews.includes(activeView) && currentUser.role !== 'admin') {
    ...
    isBlocked = false;
      ...
      isBlocked = true;
      blockMessage = { title: "Subscription Expired", body: `Your plan has expired. To continue using all features, please renew your subscription. [BUTTON]Renew Now[URL]${regUrl}` };
  }
...
[BUTTON]Register for a Full Account[URL]${regUrl}
        ...
        <header className="flex items-center justify-between p-2 border-b border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 sticky top-0 z-10">
          <div className="flex items-center gap-2">
            <button onClick={() => setIsSidebarOpen(true)} className="p-2 lg:hidden" aria-label="Open menu">
             ...
             <DynamicLogo className="w-28 text-neutral-800 dark:text-neutral-200" />
          </div>
