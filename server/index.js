...
// 🧩 MIDDLEWARE
// ===============================
// For white-labeling, allowing all origins is the most flexible approach.
// In a production environment, you might want to lock this down to a specific domain
// using an environment variable, like: `cors({ origin: process.env.ALLOWED_ORIGIN })`
app.use(cors());
...
app.get('/api/veo/download-video', async (req, res) => {
  ...
  try {
    ...
    const filename = `proxy-video-${Date.now()}.mp4`;
